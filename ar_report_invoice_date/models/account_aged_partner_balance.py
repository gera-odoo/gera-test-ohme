...
class ReportAccountAgedReceivable(models.Model):
    ...
    def print_xlsx(self, options):
        if self.env.context.get('take_invoice_date', False):
            options['take_invoice_date'] = True
            return {
        ...
        return super(ReportAccountAgedReceivable, self).print_xlsx(options=options)
    ...
    def print_pdf(self, options):
        ...
        return super(ReportAccountAgedReceivable, self).print_pdf(options=options)

    @api.model
    def _get_sql(self):
        options = self.env.context['report_options']
        if not self.env.context.get('take_invoice_date', False) and not options.get('take_invoice_date', False):
        ...
        query = ("""
                SELECT
                    ...
                    COALESCE(account_move_line.date, account_move_line.date_maturity) AS report_date,
